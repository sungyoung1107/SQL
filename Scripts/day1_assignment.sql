...
SELECT EMPNO, ENAME, SUBSTR(TO_CHAR(EMPNO),1,2)||'**' AS "EMPNO 마스킹 처리",
SUBSTR(ENAME,1,1)||'*****' AS "ENAME 마스킹 처리"
FROM EMP
WHERE LENGTH(ENAME)>=6 ;
-- /* RPAD 활용 */
SELECT EMPNO, ENAME, RPAD(SUBSTR(TO_CHAR(EMPNO),1,2),4,'*') AS "EMPNO 마스킹 처리",
RPAD(SUBSTR(ENAME,1,1),6,'*') AS "ENAME 마스킹 처리"
FROM EMP
WHERE LENGTH(ENAME)>=6 ;
